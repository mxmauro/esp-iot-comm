...
{
    addr->ip[0] = static_cast<uint8_t>(in->sin_addr.s_addr) & 0xFF;
    addr->ip[1] = static_cast<uint8_t>(in->sin_addr.s_addr >> 8) & 0xFF;
    addr->ip[2] = static_cast<uint8_t>(in->sin_addr.s_addr >> 16) & 0xFF;
    addr->ip[3] = static_cast<uint8_t>(in->sin_addr.s_addr >> 24) & 0xFF;
    addr->isIPv6 = false;
}
...
bool parseIP(IPAddress_t* addr, const char* s, size_t len)
{
    ...
    {
        pos = findChar(s, sEnd, ']');
        if ((!pos) || pos - s - 1 >= static_cast<int>(sizeof(buf)))
        {
            return false;
        }
    ...
    if (sEnd - s - 1 >= static_cast<int>(sizeof(buf)))
    {
        return false;
    }
