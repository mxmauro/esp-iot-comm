...
static Task_t udpServerTask = nullptr;
static _Atomic(int) udpServerSocket = {-1};
...
static esp_err_t deriveTransportKeys(const uint8_t sessionMasterKey[SESSION_AES_KEY_LEN], const uint8_t* salt, size_t saltLen,
                                     const uint8_t* info, size_t infoLen, uint8_t derivedKey[2 * AES_KEY_LEN + 2 * SESSION_IV_LEN]);
...
esp_err_t iotCommInit(IotCommConfig_t* config)
{
    ...
    {
        maxRequestsPerWindow = static_cast<uint8_t>((sizeof(maxRequestsPerWindow) << 8) - 1);
    }
...
esp_err_t iotCommStartServer(IotCommServerConfig_t* config)
{
    ...
    err = userIsPublicKeySet(userGetID("root", 4), &rootUserConfigured);
    if (err != ESP_OK)
    {
        return err;
    }
    if (!rootUserConfigured)
    {
        return ESP_ERR_INVALID_STATE;
    }
    ...
    serverCtx = static_cast<ServerContext_t*>(malloc(sizeof(ServerContext_t)));
    if (!serverCtx)
    {
        err = ESP_ERR_NO_MEM;
        goto on_error;
    ...
    serverCtx->maxConnectionsCount = static_cast<size_t>(config->maxConnections);
    serverCtx->maxPacketSize = (config->maxPacketSize > MIN_WS_PACKET_SIZE) ? static_cast<size_t>(config->maxPacketSize) : MIN_WS_PACKET_SIZE;
...
esp_err_t iotCommSetSessionUserData(IotCommSessionHandle_t h, void* ptr, IotCommUserDataFreeFunc_t freeFn)
{
    ...
    if (rpLock.acquired())
    {
        OnTheFlyEvent_t* otfe = static_cast<OnTheFlyEvent_t*>(h);
        ...
        switch (otfe->event->eventType)
        {
            case IotCommEventTypeSessionStart:
                if (!otfe->closeSent)
                {
                    if (reason < 400)
                    {
        reason = static_cast<uint16_t>(HTTPD_500_INTERNAL_SERVER_ERROR);
                    }
...
esp_err_t iotCommGetReadiness(IotCommReadiness_t* readiness)
{
    ...
    readiness->initialized = handler != nullptr;
    readiness->rootUserConfigured = false;
    if (!readiness->initialized)
    {
        return ESP_OK;
    }
    return userIsPublicKeySet(userGetID("root", 4), &readiness->rootUserConfigured);
}
...
esp_err_t iotCommIsRootUserConfigured(bool* configured)
{
    ...
    return userIsPublicKeySet(userGetID("root", 4), configured);
}
...
esp_err_t iotCommVerifyRootUserSignature(const uint8_t hash[P256_HASH_SIZE], const uint8_t signature[P256_SIGNATURE_SIZE])
{
    ...
    return userVerifySignature(userGetID("root", 4), hash, signature);
}
...
static esp_err_t serveWsInit(httpd_req_t* req)
{
    ...
    serverCtx = static_cast<ServerContext_t*>(httpd_get_global_user_ctx(req->handle));
    if (!serverCtx)
    {
        return ESP_ERR_INVALID_STATE;
    }
    ...
    ctx = static_cast<ServeWsInitContext_t*>(malloc(sizeof(ServeWsInitContext_t)));
    if (!ctx)
    {
    ...
    ctx->json = cJSON_ParseWithLength(reinterpret_cast<const char*>(ctx->reqBody.buffer), ctx->reqBody.used);
    ...
    if ((!fromB64(ctx->clientNonceValue, strlen(ctx->clientNonceValue), false, ctx->challenge.clientNonce, &ctx->clientNonceLen)) ||
    ...
    if (ctx->clientNonceLen != CHALLENGE_NONCE_SIZE || ctx->ecdhClientPublicKeyLen != P256_PUBLIC_KEY_SIZE ||
        (!p256ValidatePublicKey(ctx->challenge.ecdhClientPublicKey, P256_PUBLIC_KEY_SIZE)))
    {
        goto error_invalid_data;
    }

    if (randomize(ctx->challenge.serverNonce, sizeof(ctx->challenge.serverNonce)) != ESP_OK ||
        randomize(ctx->challengeCookie, sizeof(ctx->challengeCookie)) != ESP_OK || ecdhGeneratePair(&ctx->ecdhKeyPair) != ESP_OK ||
    ...
    if ((!gbAdd(&ctx->respBody, GB_STR_AND_SIZE("{\"token\":\""))) ||
        ...
        (!gbAdd(&ctx->respBody, GB_STR_AND_SIZE("\",\"maxPacketSize\":"))) || (!extGbAddSizeT(&ctx->respBody, serverCtx->maxPacketSize)) ||
    ...
    if (err == ESP_OK)
    {
        err = httpd_resp_send(req, reinterpret_cast<char*>(ctx->respBody.buffer), static_cast<ssize_t>(ctx->respBody.used));
    }
...
static esp_err_t serveWsAuth(httpd_req_t* req)
{
    ...
    ctx = static_cast<ServeWsAuthContext_t*>(malloc(sizeof(ServeWsAuthContext_t)));
    if (!ctx)
    {
        return ESP_ERR_NO_MEM;
    }
    ...
    ctx->encryptedAuth = static_cast<uint8_t*>(malloc(ctx->encryptedAuthLen));
    if (!ctx->encryptedAuth)
    {
        err = ESP_ERR_NO_MEM;
        goto done;
    }
    if ((!fromB64(ctx->cookieValue, strlen(ctx->cookieValue), false, ctx->challengeCookie, &ctx->challengeCookieLen)) ||
    ...
    ctx->plaintext = static_cast<char*>(gbReserve(&ctx->plaintextBody, ctx->plaintextLen));
    if (!ctx->plaintext)
    {
        err = ESP_ERR_NO_MEM;
        goto done;
    ...
    err = aesDecrypt(&ctx->aesCtx, ctx->encryptedAuth, ctx->encryptedAuthLen, ctx->authIv, sizeof(ctx->authIv), nullptr, 0,
                     reinterpret_cast<uint8_t*>(ctx->plaintext));
    if (err != ESP_OK)
    {
    ...
    ctx->innerJson = cJSON_ParseWithLength(reinterpret_cast<const char*>(ctx->plaintextBody.buffer), ctx->plaintextBody.used);
    ...
    if ((!fromB64(ctx->authNonceValue, strlen(ctx->authNonceValue), false, ctx->authNonce, &ctx->authNonceLen)) ||
    ...
    if ((!gbAdd(&ctx->respBody, GB_STR_AND_SIZE(",\"wsNonce\":\""))) ||
...
static esp_err_t serveWsPreHandshake(httpd_req_t* req)
{
    ...
    ctx = static_cast<ServeWsPreHandshakeContext_t*>(malloc(sizeof(ServeWsPreHandshakeContext_t)));
    if (!ctx)
    {
        return ESP_ERR_NO_MEM;
    }
    memset(ctx, 0, sizeof(ServeWsPreHandshakeContext_t));
    ctx->serverCtx = static_cast<ServerContext_t*>(httpd_get_global_user_ctx(req->handle));
    ...
    if (!tryExtractWsTicketFromQuery(reinterpret_cast<const char*>(ctx->reqQueryParams.buffer), ctx->ticketB64, sizeof(ctx->ticketB64),
                                     &ctx->carrierSelected) ||
    ...
    err = deriveTransportKeys(ctx->sessionMasterKey, ctx->salt, sizeof(ctx->salt), reinterpret_cast<const uint8_t*>(WS_TRANSPORT_INFO),
    ...
    for (SessionInfo_t* otherSession = ctx->serverCtx->sessions.first; otherSession; otherSession = otherSession->next)
    {
...
static esp_err_t serveWsPacket(httpd_req_t* req)
{
    ...
    commandCtx.serverCtx = static_cast<ServerContext_t*>(httpd_get_global_user_ctx(req->handle));
    commandCtx.serverHandle = req->handle;
    commandCtx.sockfd = httpd_req_to_sockfd(req);
    commandCtx.session = static_cast<SessionInfo_t*>(httpd_sess_get_ctx(commandCtx.serverHandle, commandCtx.sockfd));
    ...
    hdr = reinterpret_cast<PacketHeader_t*>(commandCtx.session->ciphertextIn.buffer);
    if (hdr->v != VERSION)
    {
    ...
    {
        iv[SESSION_IV_LEN - i - 1] ^= static_cast<uint8_t>((commandCtx.rxCounter >> (i << 3)) & 0xFF);
    }
    ...
    // Decrypt message
    err = aesDecrypt(&commandCtx.session->clientAesCtx, commandCtx.session->ciphertextIn.buffer + sizeof(PacketHeader_t), dataAndTagLen, iv,
                     sizeof(iv), reinterpret_cast<const uint8_t*>(hdr), sizeof(PacketHeader_t), commandCtx.session->plaintextIn.buffer);
    if (err != ESP_OK)
    {
...
static esp_err_t dispatchCommand(CommandContext_t* commandCtx)
{
    ...
    if (commandCtx->session->mustChangeCredentials != 0 && commandCtx->cmd != CMD_CHANGE_USER_CREDENTIALS)
    {
        ...
        err = closeWsWithCmdCtx(commandCtx, WS_CLOSE_APP_CREDENTIALS_CHANGE_MANDATORY, "User must change the access credentials.");
...
static esp_err_t handleChangeUserCredentials(CommandContext_t* commandCtx)
{
    ...
    if (err == ESP_OK)
    {
        err = sha256Update(&sha256Ctx, reinterpret_cast<const uint8_t*>("ws-chgcreds-v1"), 14);
        if (err == ESP_OK)
        {
...
static esp_err_t handleOtaWrite(CommandContext_t* commandCtx)
{
    ...
    if (completed)
    {
        restartErr = taskCreate(otaRestartTaskMain, "iotcomm-restart", 2048, nullptr, 5, tskNO_AFFINITY) ? ESP_OK : ESP_ERR_NO_MEM;
...
static esp_err_t handleUdpOpen(CommandContext_t* commandCtx)
{
    ...
    do
    {
        if (randomize(reinterpret_cast<uint8_t*>(&udpConnectionId), sizeof(udpConnectionId)) != ESP_OK)
        {
            ESP_LOGD(TAG, "UDP OPEN command: failed to allocate UDP negotiation state.");
            err = ESP_FAIL;
            goto on_error;
    ...
    if (err == ESP_OK)
    {
        err = sha256Update(&sha256Ctx, reinterpret_cast<const uint8_t*>("udp-open-v1"), 11);
        if (err == ESP_OK)
        {
    ...
    err = deriveTransportKeys(commandCtx->session->sessionMasterKey, udpSalt, sizeof(udpSalt), reinterpret_cast<const uint8_t*>(UDP_TRANSPORT_INFO),
    ...
    if ((!bw_write_be32(&bw, ESP_OK)) || (!bw_write_be16(&bw, commandCtx->serverCtx->udpListenPort)) ||
        (!bw_write_be32(&bw, udpConnectionId)) || (!bw_write_blob(&bw, serverUdpNonce, sizeof(serverUdpNonce))))
    {
                                ...
                                size_t plaintextOutLen, const uint64_t* replyCounter, bool closeOnError)
{
    ...
    // Header
    hdr = reinterpret_cast<PacketHeader_t*>(ciphertextOut->buffer);
    hdr->v = VERSION;
    be16enc(hdr->cmd, cmd);
    ...
    for (size_t i = 0; i < 8; i++)
    {
        iv[SESSION_IV_LEN - i - 1] ^= static_cast<uint8_t>((nextTxCounter >> (i << 3)) & 0xFF);
    }

    // Encrypt message
    err = aesEncrypt(&session->serverAesCtx, plaintextOut, plaintextOutLen, iv, SESSION_IV_LEN, reinterpret_cast<const uint8_t*>(hdr), hdrSize,
...
static void destroyServerCtx(void* ctx)
{
    if (ctx)
    {
        ServerContext_t* serverCtx = static_cast<ServerContext_t*>(ctx);
...
static void destroySessionCtx(void* ctx)
{
    if (ctx)
    {
        SessionInfo_t* session = static_cast<SessionInfo_t*>(ctx);
...
static SessionInfo_t* createSession()
{
    ...
    // Create user session
    session = static_cast<SessionInfo_t*>(malloc(sizeof(SessionInfo_t)));
    if (!session)
    {
        return nullptr;
    }
...
static esp_err_t readWsPacket(ServerContext_t* serverCtx, SessionInfo_t* session, httpd_req_t* req, bool* messageComplete)
{
    ...
    *messageComplete = false;
    ...
    if (frame.len > 0)
    {
        ...
        frame.payload = static_cast<uint8_t*>(gbReserve(&session->ciphertextIn, frame.len));
        if (!frame.payload)
        {
            return ESP_ERR_NO_MEM;
        }
...
static bool closeWs(httpd_handle_t serverHandle, int sockfd, uint16_t code, const char* reason)
{
    ...
    return httpd_ws_send_frame_async(serverHandle, sockfd, &frame) == ESP_OK && httpd_sess_trigger_close(serverHandle, sockfd) == ESP_OK;
}
...
static esp_err_t startUdpServer(ServerContext_t* serverCtx)
{
    ...
    atomic_store_explicit(&udpServerSocket, socketFd, memory_order_release);
    udpServerTask = taskCreate(udpServerTaskMain, "iotcomm-udp", 6144, serverCtx, 4, tskNO_AFFINITY);
    if (!udpServerTask)
    {
        ESP_LOGE(TAG, "Failed to start the UDP server task.");
        close(atomic_exchange_explicit(&udpServerSocket, -1, memory_order_acq_rel));
        return ESP_ERR_NO_MEM;
    }
...
static void stopUdpServer()
{
    ...
    socketFd = atomic_exchange_explicit(&udpServerSocket, -1, memory_order_acq_rel);
    if (udpServerTask)
    {
        if (socketFd >= 0)
        {
...
static void udpServerTaskMain(Task_t task, void* arg)
{
    ...
    serverCtx = static_cast<ServerContext_t*>(arg);

    taskSignalContinue(task);

    rxBuffer = static_cast<uint8_t*>(malloc(serverCtx->maxPacketSize));
    if (!rxBuffer)
    {
    ...
    while (!taskShouldQuit(task))
    {
        ...
        socketFd = atomic_load_explicit(&udpServerSocket, memory_order_acquire);
        if (socketFd < 0)
        {
            break;
        }
        len = recvfrom(socketFd, rxBuffer, serverCtx->maxPacketSize, 0, reinterpret_cast<sockaddr*>(&srcAddr), &srcAddrLen);
        ...
        {
            case AF_INET:
                parseIPv4(&remoteAddr, reinterpret_cast<const sockaddr_in*>(&srcAddr));
                break;

            case AF_INET6:
                parseIPv6(&remoteAddr, reinterpret_cast<const sockaddr_in6*>(&srcAddr));
                break;
        ...
        processUdpPacket(serverCtx, rxBuffer, static_cast<size_t>(len), &remoteAddr);
    }
...
static void processUdpPacket(ServerContext_t* serverCtx, const uint8_t* packet, size_t packetLen, const IPAddress_t* remoteAddr)
{
    ...
    hdr = reinterpret_cast<const UdpPacketHeader_t*>(packet);
    ...
    if (plaintextLen > 0)
    {
        plaintext = static_cast<uint8_t*>(malloc(plaintextLen));
        if (!plaintext)
        {
            goto done;
        }
    ...
    for (size_t i = 0; i < 8; i++)
    {
        iv[SESSION_IV_LEN - i - 1] ^= static_cast<uint8_t>((rxCounter >> (i << 3)) & 0xFF);
    }
    ...
    if (err == ESP_OK)
    {
    err = aesDecrypt(&aesCtx, packet + sizeof(UdpPacketHeader_t), ciphertextLen, iv, sizeof(iv), reinterpret_cast<const uint8_t*>(hdr),
    ...
    commandCtx.sockfd = atomic_load_explicit(&udpServerSocket, memory_order_acquire);
...
static bool extGbAddB64(GrowableBuffer_t* buf, const uint8_t* src, size_t srcLen, bool isUrl)
{
    ...
    b64Out = static_cast<char*>(gbReserve(buf, maxLen));
    if (!b64Out)
    {
        return false;
    }
...
static bool extGbAddSizeT(GrowableBuffer_t* buf, size_t value)
{
    ...
    return textLen > 0 && static_cast<size_t>(textLen) < sizeof(text) && gbAdd(buf, text, static_cast<size_t>(textLen));
}
...
static esp_err_t buildWsServerAuthHash(const Challenge_t* challenge, uint8_t hash[SHA256_SIZE])
{
    ...
    const uint8_t* parts[] = {
        label,           challenge->ecdhClientPublicKey, challenge->ecdhServerPublicKey, challenge->clientNonce, challenge->serverNonce,
        challenge->token};
    const size_t partLens[] = {sizeof(label) - 1,
...
static esp_err_t deriveWsLoginSalt(const Challenge_t* challenge, const uint8_t* extra, size_t extraLen, uint8_t salt[SHA256_SIZE])
{
    ...
    const uint8_t* parts[]       = {label, challenge->serverNonce, challenge->clientNonce, challenge->token, extra};
    const size_t partLens[] = {sizeof(label) - 1, sizeof(challenge->serverNonce), sizeof(challenge->clientNonce), sizeof(challenge->token),
                               extraLen};
...
static esp_err_t deriveSessionMasterKey(const uint8_t sharedSecret[P256_SHARED_SECRET_SIZE], const uint8_t* salt, size_t saltLen,
                                        uint8_t sessionMasterKey[SESSION_AES_KEY_LEN])
{
    return hkdfSha256DeriveKey(sharedSecret, P256_SHARED_SECRET_SIZE, salt, saltLen, reinterpret_cast<const uint8_t*>(SESSION_MASTER_INFO),
...
static esp_err_t deriveAuthEnvelopeKey(const uint8_t sharedSecret[P256_SHARED_SECRET_SIZE], const uint8_t salt[SHA256_SIZE],
                                       uint8_t authKey[AES_KEY_LEN])
{
    return hkdfSha256DeriveKey(sharedSecret, P256_SHARED_SECRET_SIZE, salt, SHA256_SIZE, reinterpret_cast<const uint8_t*>(AUTH_ENVELOPE_INFO),
...
static bool tryExtractWsTicketFromAuthorization(httpd_req_t* req, char* ticketB64, size_t ticketB64Len, bool* selected)
{
    ...
    *selected = false;
    ...
    authHeader = static_cast<char*>(malloc(authLen + 1));
    if (!authHeader)
    {
        return false;
    }
    ...
    *selected = true;
    ok = httpd_req_get_hdr_value_str(req, "Authorization", authHeader, authLen + 1) == ESP_OK && authLen > 7 &&
         memcmp(authHeader, "Bearer ", 7) == 0 && authHeader[7] != 0 && strlen(authHeader + 7) < ticketB64Len;
    if (ok)
    {
